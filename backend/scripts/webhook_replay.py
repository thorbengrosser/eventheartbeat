...
def replay(
    ...
    while i < n:
        ...
        # Occasionally send a burst of additional events back-to-back
        if i < n and burst_prob > 0 and random.random() < burst_prob:
            ...
            for _k in range(burst_count):
                ...
                ev_b = events[i]
                _ = post_json(url, ev_b)
                sent += 1
                # Update last_dt with the nominal timestamp of the burst event to keep relative pacing reasonable
